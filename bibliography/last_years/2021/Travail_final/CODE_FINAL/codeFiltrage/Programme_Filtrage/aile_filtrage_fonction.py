...
def aile_filtrage(nom_image):
    ...
    def coloriage_croix_liste(indice_x,indice_y,I,l,c,val_test,val_nouvelle):
        ...
        while L != [] and retour_trop_de_recursion==False:
            if np.sum(I==val_nouvelle)>nb_point_max:
    ...
    x_debut=int(input("Entrer la ligne de départ : "))
    x_fin=int(input("Entrer la ligne de fin : "))
    y_debut=int(input("Entrer la colonne de départ : "))
    y_fin=int(input("Entrer la colonne de fin : "))
    ...
    while(not_succeed):
        ...
        trop_de_recursion = coloriage_croix_liste(x,y,I3,l_max,c_max,255,256) # Si un point à 255 est collé 
    ...
    if rate == False:                   # Si on a réussi le filtrage, on va chercher un point noir
        ...
        cv2.imwrite(r"images_filtree\filtree_"+nom_image,I5)
