# -*- coding: utf-8 -*-
"""
Created on Wed Apr 28 00:14:06 2021
...
# num_base_token = 24
shape = np.zeros((2,nb_token)) # pour mesurer ensuite une shape moyenne à donner au token extrait

t1 = time.time()
for num in range( 1, nombre_ailes+1 ) : 
    num_aile_traitee = num  # numéro de l'aile (cf. base de donnée) dont on veut extraire le motif
    ...
    img_gray = skio.imread(filepath, as_gray = True )
    ...
    for k in range(1, nb_token+1): # on parcourt tous les tokens de la base de donnés
        ...
        dirpath_token = r'..\code\photos\token\filter\indice_cubital\indice_cubital_total\Base0'
    ...
    dirpath_token_final = r'..\code\photos\token\filter\indice_cubital\indice_cubital_total\Base0'
    ...
    plt.title('Matching Result'), plt.xticks([]), plt.yticks([])
    plt.subplot(122),plt.imshow(img_rgb2) # cmap = 'gray'
    plt.title('Detected Point'), plt.xticks([]), plt.yticks([])
    plt.suptitle("motif reconnu pour l'aile n°" + str(num_aile_traitee) + " de la ruche n°" + str(num_ruche), fontsize = 7 )

    
    fig4 = plt.figure(4)
    plt.subplot(1, 2, 1), plt.imshow(img_rgb2), plt.title("image avec le token optimal",  fontsize = 7)
    ...
    l_p = top_left2[0]
    l_f = top_left2[0] + h2
    token_optimal = decoupe(img2, l_p , h_p , l_f , h_f )
...
t2 = time.time()    
print("Temps d'extraction : " + str(round(t2-t1,2)) + ' s')